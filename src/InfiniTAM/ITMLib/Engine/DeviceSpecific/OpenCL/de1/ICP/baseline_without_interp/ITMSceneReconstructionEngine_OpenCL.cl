...
#define HESSIAN_SIZE (6 + 5 + 4 + 3 + 2 + 1)
#define SHORT_HESSIAN_SIZE (3 + 2 + 1)
		...
		global float* restrict sumF)
{
	...
	int noParaSQ = shortIteration ? SHORT_HESSIAN_SIZE : HESSIAN_SIZE;

	*noValidPoints = 0;
	*sumF = 0.f;
	for (int i = 0; i < 6; i++) sumNabla[i] = 0.0f;
	for (int i = 0; i < 6 * 6; i++) sumHessian[i] = 0.0f;
	...
	for (int y = 0; y < viewImageSize.y; y++)
	{
		for (int x = 0; x < viewImageSize.x; x++)
		{
			...
			for (int i = 0; i < 6; i++) localNabla[i] = 0.0f;
			for (int i = 0; i < HESSIAN_SIZE; i++) localHessian[i] = 0.0f;
			...
			tmp3Dpoint = matVecMul(*approxInvPose, tmp3Dpoint);
			tmp3Dpoint.w = 1.0f;

			// project into previous rendered image
			tmp3Dpoint_reproj = matVecMul(*scenePose, tmp3Dpoint);
