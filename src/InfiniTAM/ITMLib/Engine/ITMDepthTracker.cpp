...
void ITMDepthTracker::setNoIterationsPerLevel(int *iterations)
{
	for(int i = 0; i < viewHierarchy->noLevels; i++)
	{
		this->noIterationsPerLevel[i] = iterations[i];
	}
}
...
bool ITMDepthTracker::HasConverged(float *step)
{
...
void ITMDepthTracker::TrackCamera(ITMTrackingState *trackingState, const ITMView *view)
{
	...
	for (int levelId = viewHierarchy->noLevels - 1; levelId >= noICPLevel; levelId--)
	{
		...
		for (int iterNo = 0; iterNo < noIterationsPerLevel[levelId]; iterNo++)
		{
			...
			Vector3f pose_tran, pose_rot;
			trackingState->pose_d->GetParams(pose_tran, pose_rot);
			if(isnan(pose_tran.x) || isnan(pose_tran.y) || isnan(pose_tran.z)
			   || isnan(pose_rot.x) || isnan(pose_rot.y) || isnan(pose_rot.z))
			{
//				trackingState->pose_d->SetFrom(&lastKnownGoodPose);
				trackingState->pose_d->SetFrom(0.f, 0.f, 0.f, 0.f, 0.f, 0.f);
				approxInvPose = trackingState->pose_d->GetInvM();
				lambda *= 10.0f;
