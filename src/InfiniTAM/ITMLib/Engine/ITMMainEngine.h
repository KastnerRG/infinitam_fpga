...
#define NO_DEBUG_TRAJECTORY_OUTPUT
	...
	namespace Engine
	{
		...
		*/
		...
		private:
			...
			IITMVisualisationEngine *visualisationEngine_freeView;
			...
			ITMExportEngine<ITMVoxel, ITMVoxelIndex> *exportEngine;

#ifndef NO_DEBUG_TRAJECTORY_OUTPUT
			std::ofstream traj_output;
#endif

			declareProfileTimers(5); //< See NVTimer.h
		...
		public:
			...
			/// Saves the entire TSDF model to a point cloud file
			bool SaveTSDFToFile(const char *filename);

			/// Saves the poses to a text file
			bool SavePosesToFile(const char *filename);
