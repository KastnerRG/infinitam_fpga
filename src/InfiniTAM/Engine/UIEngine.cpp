...
void UIEngine::glutKeyUpFunction(unsigned char key, int x, int y)
{
	...
	case 't':
		...
		break;
	case 'w':
		printf("saving mesh to disk ..."); fflush(stdout);
		uiEngine->SaveSceneToMesh("mesh.stl");
		printf(" done\n");
		break;
	case 's':
		printf("Saving TSDF to \"world.pcd\" ..."); fflush(stdout);
		uiEngine->SaveTSDFToPCD("world.pcd");
		printf(" done\n");
		break;
	default:
		break;
...
void UIEngine::SaveTSDFToPCD(const char* filename) const
{
	mainEngine->SaveTSDFToFile(filename);
}
