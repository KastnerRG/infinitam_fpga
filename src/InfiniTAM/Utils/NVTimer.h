...
#ifndef NO_DETAILED_PROFILING

	#define declareProfileTimers(num) \
		static const int NUM_TIMERS = num; \
		StopWatchInterface * timers[num]; \
		StopWatchInterface * timers_avg[num];
	
	#define createProfileTimers \
		for(int i = 0; i < NUM_TIMERS; i++) \
		{ \
			sdkCreateTimer(&timers[i]); \
			sdkCreateTimer(&timers_avg[i]); \
		}
	
	#define deleteProfileTimers \
		for(int i = 0; i < NUM_TIMERS; i++) \
		{ \
			sdkDeleteTimer(&timers[i]); \
			sdkDeleteTimer(&timers_avg[i]); \
		}

	#define profileAndDisplayTime(func, idx, name) {\
		sdkResetTimer(&timers[idx]);\
		sdkStartTimer(&timers[idx]);\
		sdkStartTimer(&timers_avg[idx]);\
		func;\
		sdkStopTimer(&timers[idx]);\
		sdkStopTimer(&timers_avg[idx]);\
		float m_time     = sdkGetTimerValue(&timers[idx]);\
		float m_time_avg = sdkGetAverageTimerValue(&timers_avg[idx]);\
		std::cout << name << ": " << m_time << "ms (Average: " << m_time_avg << "ms)\n";\
	}

#else

	#define profileAndDisplayTime(func, idx, name) {func;}
	#define declareProfileTimers(num) 
	#define createProfileTimers 
	#define deleteProfileTimers 
