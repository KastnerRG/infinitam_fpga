...
#define USE_CL_HOST_PTR // TODO make OpenCL more general
...
namespace ORUtils
{
	...
	*/
...
#ifdef COMPILE_WITH_OPENCL
		/** Pointer to OpenCL memory, if available. */
		cl_mem data_opencl;
#endif
	...
	public:
		...
		/** Get the OpenCL data pointer. */
		inline cl_mem GetOpenCLData()
		{
			return data_opencl;
		}
#endif
		...
		void Allocate(size_t dataSize, bool allocate_CPU, bool allocate_CUDA, bool metalCompatible)
		{
			...
			if (allocate_CPU)
			{
				...
				case 0:
					...
					{
#ifdef USE_CL_HOST_PTR
						OpenCLContext& context = OpenCLContext::getInstance();
						if(!context.isInitialized()){ std::cerr << "OpenCL not initialized!" << std::endl; exit(EXIT_FAILURE); } // TODO no exit
						cl_int error;
						data_opencl = clCreateBuffer(
								context.cl_context_.context,
								CL_MEM_ALLOC_HOST_PTR, dataSize * sizeof(T),
								NULL, &error); // TODO error code
						clCheckErr(error, "Create buffer");
						data_cpu = static_cast<DEVICEPTR(T)*>(
								clEnqueueMapBuffer(
										context.cl_context_.queues[0],
										data_opencl,
										CL_TRUE, CL_MAP_READ | CL_MAP_WRITE,
										0, dataSize * sizeof(T),
										0, NULL, NULL, &error));
						clCheckErr(error, "Map OpenCL buffer");
#else
						data_cpu = new T[dataSize];
//						posix_memalign ((void**)&data_cpu, 64, dataSize * sizeof(T));
#endif
					}
					break;
				case 1:
...
#ifdef USE_CL_HOST_PTR
						OpenCLContext& context = OpenCLContext::getInstance();
						if(!context.isInitialized()){ std::cerr << "OpenCL not initialized!" << std::endl; exit(EXIT_FAILURE); }
						clEnqueueUnmapMemObject(context.cl_context_.queues[0], data_opencl, data_cpu, 0, NULL, NULL);
						clReleaseMemObject(data_opencl);
#else
						delete[] data_cpu;
//						free(data_cpu);
#endif
